...
<div class="layui-body">
	  ...
	  <div class="layui-tab-content">
	  	   <div class="layui-tab-item layui-show">
				<table class="layui-table" id="menuTable">
	                ...
	                <tbody>
	                <?php $num = 0;foreach ($this->getVar('menus') as $key => $value) { $num++;?>
	                    <tr data-tt-id='<?php echo $value->mcode; ?>' data-tt-parent-id="<?php echo $value->pcode; ?>">
	                        <td><?php echo $num; ?></td>
	                        <td>
	                            <i class="fa <?php echo $value->ico; ?>" aria-hidden="true"></i> 
	                            <?php echo $value->name; ?></td>
	                        <td><?php echo $value->mcode; ?></td>
	                        ...
	                        <td>
	                            <?php if ($value->status) {?>
	                            <a href="<?php echo \core\basic\Url::get('/admin/'.C.'/mod/mcode/'.$value->mcode.'/field/status/value/0');?>"><i class='fa fa-toggle-on' title="点击关闭显示"></i></a>
	                            <?php } else { ?>
	                            <a href="<?php echo \core\basic\Url::get('/admin/'.C.'/mod/mcode/'.$value->mcode.'/field/status/value/1');?>"><i class='fa fa-toggle-off' title="点击开启显示"></i></a>
	                            <?php } ?>
	                        </td>
	                        
	                        <td>
	                            <?php echo get_btn_del($value->mcode,'mcode');?>
	                            <?php echo get_btn_mod($value->mcode,'mcode');?>
	                        </td>
	                    </tr>
	                <?php } ?>
	                </tbody>
          ...
          <div class="layui-tab-item">
            <form action="<?php echo \core\basic\Url::get('/admin/Menu/add');?>" method="post" class="layui-form">
                <input type="hidden" name="formcheck" value="<?php echo $this->getVar('formcheck');?>" > 
				    ...
				    <div class="layui-input-block">
				      <select name="pcode" lay-verify="required">
				        <option value="0">顶级菜单 </option>
                       	 <?php echo $this->getVar('menu_select');?>
				      </select>
				    </div>
				 </div>
                ...
                <div class="layui-form-item">
                     ...
                     <div class="layui-input-block">
                     	<?php $num = 0;foreach ($this->getVar('actions') as $key => $value) { $num++;?>
                     	<input type="checkbox" name="actions[]" value="<?php echo $value->value; ?>" title="<?php echo $value->item; ?>">
                     	<?php } ?>
                     </div>
                </div>                 
	...
	<div class="layui-tab layui-tab-brief" lay-filter="tab">
	  ...
	  <div class="layui-tab-content">
	  	<div class="layui-tab-item layui-show">
	  	 <form action="<?php echo \core\basic\Url::get('/admin/Menu/mod/mcode/'.get('mcode').'');?><?php echo $this->getVar('backurl');?>" method="post" class="layui-form">
                <input type="hidden" name="formcheck" value="<?php echo $this->getVar('formcheck');?>" > 
                <div class="layui-form-item">
				    <label class="layui-form-label">父菜单</label>
				    <div class="layui-input-block">
				 ...
				 <div class="layui-form-item">
                     <label class="layui-form-label">菜单名称</label>
                     <div class="layui-input-block">
                     	<input type="text" name="name" required  lay-verify="required" value="<?php echo @$this->getVar('menu')->name;?>"  placeholder="请输入菜单名称"  class="layui-input">
                     </div>
                </div>
				 ...
				 <div class="layui-form-item">
                     <label class="layui-form-label">URL</label>
                     <div class="layui-input-block">
                     	<input type="text" name="url" value="<?php echo @$this->getVar('menu')->url;?>"  placeholder="请输入菜单URL"   class="layui-input">
                     </div>
                </div>
				...
				<div class="layui-form-item">
                     <label class="layui-form-label">排序</label>
                     <div class="layui-input-block">
                     	<input type="text" name="sorting" value="<?php echo @$this->getVar('menu')->sorting;?>"   placeholder="请输入菜单排序" value="255"  class="layui-input">
                     </div>
                </div> 
                 ...
                 <div class="layui-form-item">
                     <label class="layui-form-label">状态</label>
                     <div class="layui-input-block">
                     	<input type="radio" name="status" value="1" title="显示" <?php if ($this->getVar('menu')->status==1) {?>checked="checked"<?php } ?>>
						<input type="radio" name="status" value="0" title="隐藏" <?php if ($this->getVar('menu')->status==0) {?>checked="checked"<?php } ?>>
                     </div>
                </div>                               
                ...
                <div class="layui-form-item">
                     <label class="layui-form-label">桌面快捷图</label>
                     <div class="layui-input-block">
                     	<input type="radio" name="shortcut" value="1" title="是" <?php if ($this->getVar('menu')->shortcut==1) {?>checked="checked"<?php } ?>>
						<input type="radio" name="shortcut" value="0" title="否" <?php if ($this->getVar('menu')->shortcut==0) {?>checked="checked"<?php } ?>>
                     </div>
                </div>
 				...
 				<div class="layui-form-item">
                     <label class="layui-form-label">快捷图标</label>
                     <div class="layui-input-block">
                     	<input type="text" name="ico" value="<?php echo @$this->getVar('menu')->ico;?>"  placeholder="请输入菜单快捷图标"  class="layui-input">
                     </div>
                </div>  
                ...
                <div class="layui-form-item">
                     ...
                     <div class="layui-input-block">
                     	<?php $num = 0;foreach ($this->getVar('actions') as $key => $value) { $num++;?>
                     	<input type="checkbox" name="actions[]" value="<?php echo $value->value; ?>"  <?php if (in_array($value->value,$this->getVar('menu')->actions)) {?>checked="checked"<?php } ?> title="<?php echo $value->item; ?>">
                     	<?php } ?>
                     </div>
                </div>                 
...
<script type="text/javascript" src="<?php echo APP_THEME_DIR;?>/js/mylayui.js?v=v3.0.6"></script>
...
<?php return array (
  0 => '/www/wwwroot/test.admin868.com/apps/admin/view/default/common/head.html',
  1 => '/www/wwwroot/test.admin868.com/apps/admin/view/default/common/foot.html',
); ?>